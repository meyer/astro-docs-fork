...
Set `compressHTML` to `'jsx'` to enable JSX-style whitespace stripping, using the same algorithm that Babel, TypeScript, SWC, and esbuild use when compiling JSX. Whitespace stripping is more aggressive and whitespace must be explicitly preserved using expression containers (`{" "}`).

This mode is useful when using Astro with frameworks that use JSX (e.g. React). Component formatting stays the same in Astro and your JSX framework of choice.
